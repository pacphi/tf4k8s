...
pe git clone https://github.com/pacphi/tf4k8s
pe cd tf4k8s/bom
...
prompt Sign-up and authntication
...
p "Next, we'll want to create a service account and assign a role. For the purposes of this demo, "
...
pe gcloud iam service-accounts create $GCP_SERVICE_ACCOUNT
pe gcloud projects add-iam-policy-binding $GCP_PROJECT --member="serviceAccount:$GCP_SERVICE_ACCOUNT@$GCP_PROJECT.iam.gserviceaccount.com" --role="roles/owner"
wait
...
pe gcloud iam service-accounts keys create $GCP_SERVICE_ACCOUNT.$GCP_PROJECT.json --iam-account=$GCP_SERVICE_ACCOUNT@$GCP_PROJECT.iam.gserviceaccount.com
mkdir -p ~/.tf4k8s/gcp
mv $GCP_SERVICE_ACCOUNT.$GCP_PROJECT.json ~/.tf4k8s/gcp
wait
...
p "Of course, all these steps, and any pre-requisites, are explained in the README of each experiment."
p "So, let's see what we need to do."
pe cat simple-cluster.tf
pe cat terraform.tfvars.sample
pe cp terraform.tfvars.sample terraform.tfvars
...
pe sed -i "s/terraform-my-project-service-account-credentials.json/$GCP_SERVICE_ACCOUNT.$GCP_PROJECT.json/g" terraform.tfvars
pe sed -i "s/my-project/$GCP_PROJECT/g" terraform.tfvars
pe sed -i "s/k8s/$K8S_ENV/g" terraform.tfvars
pe sed -i 's/3/8/g' terraform.tfvars
pe sed -i 's/n1-standard-4/e2-standard-4/g' terraform.tfvars
pe cat terraform.tfvars
wait
p "Ready, set, go..."
pe "./create-cluster.sh"
...
pe export KUBECONFIG=$(terraform output path_to_kubeconfig)
...
pe sed -i "s/terraform-my-project-service-account-credentials.json/$GCP_SERVICE_ACCOUNT.$GCP_PROJECT.json/g" terraform.tfvars
pe sed -i "s/my-project/$GCP_PROJECT/g" terraform.tfvars
pe sed -i "s/some-zone/$BASE_NAME-zone/g" terraform.tfvars
pe sed -i "s/foo/$SUB_NAME/g" terraform.tfvars
pe sed -i "s/prefix/$SUB_NAME/g" terraform.tfvars
pe cat terraform.tfvars
pe ./create-dns.sh
...
pe sed -i "s/terraform-my-project-service-account-credentials.json/$GCP_SERVICE_ACCOUNT.$GCP_PROJECT.json/g" terraform.tfvars
pe sed -i "s/some.domain.com/$SUB_DOMAIN/g" terraform.tfvars
pe sed -i "s/my-project/$GCP_PROJECT/g" terraform.tfvars
pe sed -i "s/your@email.com/$EMAIL_ADDRESS/g" terraform.tfvars
pe sed -i "s/a-zone/$SUB_NAME-zone/g" terraform.tfvars
pe sed -i "s+~/.kube/config+$KUBECONFIG+g" terraform.tfvars
pe cat terraform.tfvars
pe ./create-certmanager.sh
p "Let's verify it is deployed correctly by checking the [ cert-manager ] namespace for running pods."
pe kubectl get pods --namespace cert-manager
wait
...
pe cd ../../gcp/external-dns
pe cat external-dns.tf
pe cat terraform.tfvars.sample
pe cp terraform.tfvars.sample terraform.tfvars
...
p "Terraform will delegate to the Habor Helm chart to install it."
pe cd ../../k8s/harbor
pe cat harbor.tf
pe cat terraform.tfvars.sample
pe cp terraform.tfvars.sample terraform.tfvars
...
p "Take note of the them.  Verify you can login to Harbor by visiting the URL in your favorite browser."
...
pe export HARBOR_ENDPOINT=$(terraform output harbor_endpoint)
pe export HARBOR_ADMIN_PASSWORD=$(terraform output harbor_admin_password)
wait
...
pe ./create-tas4k8s gcp $TANZU_NETWORK_API_TOKEN
p "Congratulations! You've just up-leveled your Kubernetes game and provided a foundation your developers will love!"
p "Let's test it out."
pe cf api $(terraform output tas_api_endpoint)
pe cf auth $(terraform output tas_admin_username) $(terraform output tas_admin_password)
